...
@onready var _save_button: Button = $PlayEndedPanel/VBox/SaveButton
...
func _ready() -> void:
	...
	GameManager.over_limit.connect(_refresh_ghost_list)
...
func _update_panels(state: GameManager.GameState) -> void:
	...
	# 枠が一杯のときは保存不可（削除を促す）
	_save_button.disabled = LoopManager.is_at_limit

func _refresh_ghost_list() -> void:
	for child in _ghost_list.get_children():
		child.queue_free()

	# 念のため上限で頭打ち（通常 ghost_count は max_ghosts を超えない）
